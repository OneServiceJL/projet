<!-- Sidebar -->
<div class="sidebar" id="sidebar">
	<div class="sidebar-inner slimscroll">
		<div id="sidebar-menu" class="sidebar-menu">
			<ul>
				<li class="submenu-open">
					<h6 class="submenu-hdr">Main</h6>
					<ul>
						<li class="submenu">
							<a href="javascript:void(0);" class="subdrop active"><i data-feather="grid"></i><span>Dashboard</span><span class="menu-arrow"></span></a>
							<ul>
								<li><a href="index.php" class="active">Admin Dashboard</a></li>
							</ul>
						</li>
						<li class="submenu">
							<a href="javascript:void(0);"><i data-feather="smartphone"></i><span>My Workspace</span><span class="menu-arrow"></span></a>
							<ul>
								<li><a href="modules/chat/index.php">Chat Group</a></li>
								<li class="submenu submenu-two"><a href="javascript:void(0);">Tasks<span class="menu-arrow inside-submenu"></span></a>
									<ul>
										<li id="quote-generator"><a href="modules/quote-generator.php">Quote generator</a></li>
										<li id="event-order"><a href="modules/event-order.php">Event order</a></li>
										<li id="transport-fleet-record"><a href="modules/transport-fleet-record.php">Transport fleet record</a></li>
										<li id="equipment-records"><a href="modules/equipment-records.php">Equipment records</a></li>
										<li id="finance-record-expence-income"><a href="modules/finance-record-expence-income.php">Finace record expence, income</a></li>
										<li id="personal-report"><a href="modules/personal-report.php">Personal repport</a></li>
										<li id="recruitment"><a href="modules/recruitment.php">Recruitement</a></li>
										<li id="employee-management"><a href="modules/employee-management.php">Employees Mangment</a></li>  
										<li id="leave-management"><a href="modules/leave-management.php">Leave Managment</a></li>
										<li id="market-analysis"><a href="modules/market-analysis.php">Market analysis</a></li>

									</ul>
								</li>
								<li><a href="modules/email.php">Email</a></li>
								<li><a href="modules/file-manager.php">Shared Files Manager</a></li>
							</ul>
						</li>
					</ul>
				</li>
				<li class="submenu-open">
					<h6 class="submenu-hdr">MANAGEMENT</h6>
					<ul>
						<li class="submenu submenu-two"><a href="javascript:void(0);">Administration & Finance<span></span></a>
							<ul>
								<li><a href="modules/administration-finance.php">Administration & Finance</a></li>
								<li><a href="modules/stock-management.php">Stock Management</a></li>
								<li><a href="modules/inventory-management.php">Inventory Management</a></li>
								<li><a href="modules/order-management.php">Order Management</a></li>
								<li><a href="modules/supplier-management.php">Supplier Management</a></li>
								<li><a href="modules/customer-management.php">Customer Management</a></li>
								<li><a href="modules/project-management.php">Project Management</a></li>
								<li><a href="modules/task-management.php">Task Management</a></li>
								<li><a href="modules/report-management.php">Report Management</a></li>
								<li><a href="modules/setting-management.php">Setting Management</a></li>
							</ul>
						</li>
						<li class="submenu submenu-two"><a href="javascript:void(0);">Operations tasks<span></span></a>
							<ul>
								<li><a href="modules/venue-booking.php">Venue booking</a></li>
								<li><a href="modules/equipment-checklist.php">Equipment checklist</a></li>
								<li><a href="modules/expense-report.php">Expense report</a></li>
								<li><a href="modules/mobile-crash-monitoring.php">Mobile crash monitoring</a></li>
								<li><a href="modules/shared-files-manager.php">Shared Files Manager</a></li>
								<li><a href="modules/chat-group.php">Chat Group</a></li>
								<li><a href="modules/email.php">Email</a></li>
							</ul>
						</li>
						<li class="submenu submenu-two"><a href="javascript:void(0);">Business Development<span></span></a>
							<ul>
								<li><a href="modules/custom-playbooks.php">Custom management books</a></li>
								<li><a href="modules/business-card-scanning.php">Business scanning</a></li>
								<li><a href="modules/contract-management.php">Contract management</a></li>
								<li><a href="modules/digital-asset-management.php">Digital asset management</a></li>
								<li><a href="modules/guest-contributor-portal.php">Guest contributor portal</a></li>
								<li><a href="modules/google-analytics.php">Google Analytics</a></li>
								<li><a href="modules/performance-tracking.php">Performance Tracking</a></li>
							</ul>
						</li>
						<li class="submenu submenu-two"><a href="javascript:void(0);">Human Resources<span></span></a>
							<ul>
								<li><a href="modules/employee-database.php">Employee database</a></li>
								<li><a href="modules/leave-management.php">Leave management</a></li>
								<li><a href="modules/recruitment.php">Recruitment</a></li>
								<li><a href="modules/attendance-tracking.php">Attendance tracking</a></li>								
								<li><a href="modules/performance-evaluations.php">Performance evaluations</a></li>
								<li><a href="modules/payroll-processing.php">Payroll processing</a></li>								
								<li><a href="modules/training-and-development.php">Training and development</a></li>
								<li><a href="modules/documents-repository-centralized.php">Documents Repository Centralized</a></li>															
							</ul>
						</li>
						<li class="submenu submenu-two"><a href="javascript:void(0);">Engineering<span></span></a>	
							<ul>
								<li><a href="modules/role-based-permissions.php">Role-based permissions</a></li>
								<li><a href="modules/user-activity-logs.php">User activity logs</a></li>
								<li><a href="modules/ip-restrictions.php">IP restrictions</a></li>
								<li><a href="modules/user-management.php">User management</a></li>
							</ul>
						</li>
					</ul>
				</li>
				<li class="submenu-open">
					<h6 class="submenu-hdr">SALES</h6>
					<ul>
						<li><a href="modules/energy-radio.php"><i data-feather="package"></i><span>Energy Radio</span></a></li>
						<li><a href="modules/top-5-music.php"><i data-feather="clipboard"></i><span>Top 5 Music</span></a></li>
						<li><a href="modules/5-start-event.php"><i data-feather="truck"></i><span>5 Start Event</span></a></li>
						<li><a href="modules/top-5-films.php"><i data-feather="shopping-cart"></i><span>Top 5 Films</span></a></li>
						<li><a href="modules/top-5-images.php"><i data-feather="file-text"></i><span>Top 5 Images</span></a></li>
						<li><a href="modules/top-5-coms.php"><i data-feather="copy"></i><span>Top 5 Coms</span></a></li>
						<li><a href="modules/top-5-technologies.php"><i data-feather="save"></i><span>Top 5 Technologies</span></a></li>
						<li><a href="modules/online-broadcasting.php"><i data-feather="hard-drive"></i><span>Online Broadcasting</span></a></li>
						<li><a href="modules/general-services.php"><i data-feather="hard-drive"></i><span>General Services</span></a></li>
					</ul>
				</li>
				<li class="submenu-open">	
					<h6 class="submenu-hdr">BRANCHES</h6>
					<ul>
						<li><a href="modules/kigali.php"><i data-feather="package"></i><span>KIGALI</span></a></li>
						<li><a href="modules/musanze-isonga.php"><i data-feather="truck"></i><span>MUSANZE (ISONGA)</span></a></li>
						<li><a href="modules/musanze-energy-radio.php"><i data-feather="shopping-cart"></i><span>MUSANZE (ENERGY RADIO)</span></a></li>
					</ul>
				</li>
			</ul>
		</div>
	</div>
</div>
<!-- /Sidebar -->